...
export default function PlanningAdmin() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans">
      ...
      // --- EXTRAIT CORRIGÉ DE LA POPUP DANS page.tsx ---
    ...
    <div className="bg-white rounded-[40px] p-8 max-w-lg w-full shadow-2xl text-slate-900">
        ...
        {activeTab === 'reserver' ? (
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input id="pop_name" type="text" defaultValue={selectedEvent.title?.split('📞')[0].trim()} className="w-full p-4 mt-1 rounded-2xl bg-slate-50 border-none font-bold outline-none" />
            </div>
            <div>
              <label className="text-[9px] font-black uppercase text-slate-400 ml-1">Téléphone</label>
              <input id="pop_phone" type="text" defaultValue={selectedEvent.extendedProps.notes} className="w-full p-4 mt-1 rounded-2xl bg-slate-50 border-none font-bold" />
            </div>
            <div>
              <label className="text-[9px] font-black uppercase text-slate-400 ml-1">Moniteur</label>
              <select id="pop_monitor" className="w-full p-4 mt-1 rounded-2xl bg-slate-50 border-none font-bold">
                {monitors.map(m => (
            ...
            <button 
              onClick={() => {
                ...
                handleUpdate({ title: name, notes: phone, monitor_id: monId, status: 'booked' });
              }} 
          ...
          <div className="space-y-4">
            <textarea id="pop_notes_text" defaultValue={selectedEvent.extendedProps.notes} placeholder="Motif..." className="w-full p-4 h-32 rounded-3xl bg-slate-50 border-none font-bold resize-none"></textarea>
            <div className="grid grid-cols-2 gap-3">
              ...
              >Bloquer Pilote</button>
              <button 
                onClick={async () => {
                  const note = (document.getElementById('pop_notes_text') as HTMLTextAreaElement).value;
                  await apiFetch('/api/admin/appointments/block-all', {
                    method: 'POST',
      ...
      <div className="mt-6 flex flex-col gap-2">
        {selectedEvent.extendedProps.status === 'booked' && (
            <button onClick={async () => { if(confirm("Libérer ?")) { await apiFetch(`/api/admin/appointments/${selectedEvent.id}/cancel`, { method: 'DELETE' }); setShowEditModal(false); loadData(); setCalendarKey(k=>k+1); }}} className="text-rose-500 font-black text-[10px] uppercase">Réinitialiser le créneau</button>
        )}
