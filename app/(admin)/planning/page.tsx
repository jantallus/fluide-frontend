...
export default function PlanningAdmin() {
  ...
  const toggleGoogleSync = async () => {
    ...
    try {
      ...
      toast.info(newValue ? "Synchronisation Google Agenda ACTIVÉE" : "Synchronisation Google Agenda DÉSACTIVÉE");
  ...
  const handleSaveNote = async () => {
    ...
    if (activeTab === 'note') {
      ...
      if (!isNonBlockingNote) {
        ...
        if (hasClientBooking) { toast.error("Impossible de bloquer : un ou plusieurs clients sont déjà réservés."); return; }
      }
    } else {
      if (!formData.title || formData.title.trim() === '') { toast.warning("Le nom du contact est obligatoire pour une réservation."); return; }
      if (!formData.flight_type_id) { toast.warning("Veuillez choisir un type de vol."); return; }
      if (!formData.phone || formData.phone.trim() === '') { toast.warning("Le numéro de téléphone est obligatoire."); return; }
    ...
    if (activeTab === 'note') {
      ...
      if (groupSize > 1 || isManual) {
        if (!displayDistribution || !displayDistribution.canFit || displayDistribution.slotsToUse.length === 0) {
          toast.error("Pas assez de créneaux disponibles ou aucune place sélectionnée."); return;
        }
  ...
  const handleMove = async () => {
    ...
    if (moveGroup && groupRootSlots.length > 1) {
      ...
      if (isExactlySame) { toast.info("Le groupe est déjà assigné exactement à ces mêmes créneaux et pilotes."); return; }
      ...
      if (targetSlot.id === selectedEvent.id) {
        toast.info("Le créneau est déjà à cet emplacement avec ce pilote."); return;
      }
        ...
        if (slotToBook) {
          updatesToApply.push({ 
            ...
            data: { 
              ...
              payment_data: selectedEvent.payment_data
            } 
          });
        }
      }
                  ...
                  const sendGenerationRequest = async (force = false) => {
                    try {
                      ...
                      if (res.status === 409 && data.warning) {
                        const userConfirmed = await confirm(data.message);
                      ...
                      if (res.ok) {
                        ...
                        await loadAppointments();
                      } else { toast.error("Erreur : " + (data.error || "Erreur inconnue")); }
