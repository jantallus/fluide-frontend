...
export default function PlanningAdmin() {
  ...
  const {
    appointments, setAppointments, monitors, flightTypes, openingPeriods,
    slotDefs, availablePlans, timeBounds, isGoogleSyncEnabled,
  ...
  const handleEventClick = useCallback((info: any) => {
    ...
    setSelectedEvent({
      id: event.id, title: event.extendedProps.title, start: event.start,
      monitor_id: event.getResources()[0]?.id, ...event.extendedProps,
    });
