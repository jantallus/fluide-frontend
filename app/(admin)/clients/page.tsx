...
export default function ClientsPage() {
  ...
  // 🎯 NOUVEAU : Mémoires pour le paiement partagé (Reste à payer)
  ...
  const saveQuickEdit = async (slotId: number, clientId: number) => {
    ...
    if (editType === 'monitor') {
      ...
      // 🎯 NOUVEAU : Création intelligente de la ligne comptable (Paiement 1 + Paiement 2)
      let part1 = "";
      if (tempPayMethod === 'Bon Cadeau' || tempPayMethod === 'Promo') {
        const codeText = tempPayCode ? ` - Code: ${tempPayCode.toUpperCase()}` : '';
        part1 = `${tempPayMethod}${codeText} - ${tempPayAmount}€`;
      } else {
        part1 = `${tempPayMethod} - ${tempPayAmount}€`;
      }
 ...
 useEffect(() => {
  ...
  const renderClientTable = (title: string, clientsList: any[], icon: string, bgColor: string, textColor: string) => {
    ...
    const renderPaymentEditor = (f: any, c: any) => {
      ...
      return (
        // 🎯 1. On retire le min-w forcé sur mobile (remplacé par md:min-w)
        <div className="space-y-2 w-full md:min-w-[220px]" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex gap-2">
            <select
              ...
              // 🎯 2. min-w-0 permet au menu de rétrécir sans casser le design
            ...
            <input
              ...
              // 🎯 3. shrink-0 empêche la case du prix de se faire écraser
                  ...
                  {c.flights.map((f: any) => (
                    <div key={f.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm space-y-3 relative">
                      ...
                      {editingSlotId === f.id && (
                        ...
                        <div className="relative mt-3 bg-white border border-slate-200 z-10 flex flex-col p-3 rounded-2xl shadow-xl animate-in fade-in slide-in-from-top-2" onClick={e => e.stopPropagation()}>
