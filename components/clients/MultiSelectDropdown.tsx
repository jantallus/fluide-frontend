...
export function MultiSelectDropdown({ label, icon, options, selected, onChange }: MultiSelectDropdownProps) {
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 max-h-64 overflow-y-auto p-2 flex flex-col gap-1 animate-in fade-in">
          {options.map((o, idx) => (
            <label
              key={idx}
              className="flex items-center gap-3 p-2 hover:bg-slate-50 cursor-pointer rounded-xl text-xs font-bold text-slate-700 transition-colors"
            >
              <input
                type="checkbox"
          ...
          {options.length === 0 && (
            <p className="text-xs text-center text-slate-400 p-2">Aucune option</p>
          )}
        </div>
      )}
    </div>
